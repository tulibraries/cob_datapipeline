"""Airflow DAG to perform a partial index of tul_cob catalog from OAI into SolrCloud."""
...
def create_dag(env, version):
    ...
    CATALOG_HARVEST_FROM_DATE = Variable.get(f"CATALOG_{env.upper()}_HARVEST_FROM_DATE")
    ...
    GIT_BRANCH = Variable.get(f"CATALOG_{env.upper()}_BRANCH")
    LATEST_RELEASE = Variable.get(f"CATALOG_{env.upper()}_LATEST_RELEASE")

    # Get Solr URL & Collection Name for indexing info; error out if not entered
    SOLR_CONN = BaseHook.get_connection("SOLRCLOUD-WRITER")
    CATALOG_SOLR_CONFIG = Variable.get(f"CATALOG_OAI_HARVEST_SOLR_CONFIG_{env.upper()}", deserialize_json=True)
    ...
    with DAG:
        ...
        UPDATE_DATE_VARIABLES = PythonOperator(
            ...
            op_kwargs={
                "UPDATE": {
                    f"CATALOG_{env.upper()}_HARVEST_FROM_DATE": CATALOG_HARVEST_FROM_DATE_NEW
                }
            },
            dag=DAG
        )
...
STAGE_DAG = create_dag("stage", "1.0.0")
