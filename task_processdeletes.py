"""TU Libraries Airflow Task for Alma to Solr Record Deletes."""
...
def process_deletes(**kwargs):
    """Method to Process Deletes from Alma OAI Feed to apply Delete to Solr."""
    num_deleted = -1
    ...
    with open(deletes_fname) as fd:
        ...
        if doc is not None and 'record' in doc['collection']:
            for record in doc['collection']['record']:
                #oai:alma.01TULI_INST:991000011889703811 (record header)
                if 'ns0:header' in record:
                    ...
                    if 'ns0:identifier' in header:
                        ...
                        if len(header['ns0:identifier'].split(':')) > 2:
                            record_id = header['ns0:identifier'].split(':')[2]
                            #2018-09-21T20:21:12Z (datestamp)
                            if 'ns0:datestamp' in header:
                                date = header['ns0:datestamp']
                                param_endpoint_update_delete = '/solr/' + kwargs.get('core_name') + "/update?stream.body=<delete><query>record_update_date:[0 TO {}] AND id:{}</query></delete>&commit=true".format(date, record_id)
                                print(record_id)
                                method = 'GET'
                                http_conn_id = 'AIRFLOW_CONN_SOLR_LEADER'
                                data = {"command": 'delete_{}'.format(record_id)}
