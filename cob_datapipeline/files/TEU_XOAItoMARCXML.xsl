...
<xsl:stylesheet version="2.0" xmlns:dc="http://purl.org/dc/elements/1.1/"
    ...
    <xsl:template match="element[@name = 'dc']">
        <record>
            ...
            <xsl:variable name="thesisDate"
                select="element[@name = 'date']/element[@name = 'issued']/element/field[@name = 'value']"/>
            <xsl:variable name="thesisYear">
                <xsl:value-of select="substring($thesisDate, 1, 4)"/>
            </xsl:variable>

            <!-- Added a lookup to change DSpace language 'es' to 'spa' in MARC 008/35-37 for Spanish; add additional language mappings to lookup as needed.xsl -->
            <xsl:variable name="thesisLanguage">
                <xsl:choose>
                    <xsl:when
                        test="normalize-space(element[@name = 'language']/element[@name = 'iso']/element/field[@name = 'value']) != ''">
                        <xsl:call-template name="language">
                            <xsl:with-param name="languageCode"
                                select="element[@name = 'language']/element[@name = 'iso']/element/field[@name = 'value']"/>
                        </xsl:call-template>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of select="(.)"/>
            ...
            <xsl:variable name="thesisTitle"
                select="element[@name = 'title']/element/field[@name = 'value'][1]"/>
            <xsl:variable name="thesisAuthor"
                select="element[@name = 'creator']/element/field[@name = 'value']"/>
            <xsl:variable name="authorORCID"
                select="element[@name = 'creator']/element[@name = 'orcid']/element/field[@name = 'value'][1]"/>
            <xsl:variable name="embargo"
                select="element[@name = 'embargo']/element[@name = 'lift']/element/field[@name = 'value']"/>
            <xsl:variable name="hdl"
                select="element[@name = 'identifier']/element[@name = 'uri']/element/field[@name = 'value']"/>
            <xsl:variable name="doi"
                select="element[@name = 'relation']/element[@name = 'doi']/element/field[@name = 'value']"/>
            <xsl:variable name="currentDateTrim"
                select="translate(substring(string(current-date()), 3, 8), '-:T.', '')"/>
            <xsl:variable name="currentDateTimeTrim"
            ...
            <xsl:if test="element[@name = 'identifier']/element[@name = 'filename']">
                ...
                <xsl:variable name="filename"
                    select="element[@name = 'identifier']/element[@name = 'filename']/element/field[@name = 'value']"/>
            ...
            <xsl:if test="element[@name = 'identifier']/element[@name = 'proqst']">
                ...
                <datafield tag="035" ind1=" " ind2=" ">
                    <subfield code="a">
                        ...
                        <xsl:value-of
                            select="element[@name = 'identifier']/element[@name = 'proqst']/element/field[@name = 'value']"
                        />
                    </subfield>
                </datafield>
            ...
            <xsl:variable name="itemID"
                select="replace(substring-after($doi, 'doi.org/'), '[^a-zA-Z0-9\-:_]', '_')"/>
            ...
            <datafield tag="245" ind1="1">
                ...
                <xsl:if test="contains($thesisTitle, ':')">
                    <subfield code="a">
                        <xsl:value-of select="substring-before($thesisTitle, ':')"/>
                        <xsl:text xml:space="preserve"> : </xsl:text>
                    </subfield>
                    <subfield code="b">
                        <xsl:value-of select="substring-after($thesisTitle, ':')"/>
                        <xsl:text xml:space="preserve"> /</xsl:text>
                    </subfield>
                    <subfield code="c">
            ...
            <datafield tag="264" ind1=" " ind2="1">
                ...
                <subfield code="c">
                    <xsl:value-of select="$thesisYear"/>. </subfield>
            </datafield>

            <!-- Physical Description -->

            <xsl:variable name="extent"
                select="element[@name = 'format']/element[@name = 'extent']/element/field[@name = 'value']"/>
            ...
            <datafield tag="502" ind1=" " ind2=" ">
                <subfield code="b">
                    <xsl:value-of
                        select="element[@name = 'description']/element[@name = 'degree']/element/field[@name = 'value']"
                    />
                </subfield>
            ...
            <xsl:if test="element[@name = 'description']/element[@name = 'department']">
                <datafield tag="500" ind1=" " ind2=" ">
                    <subfield code="a">
                        ...
                        <xsl:value-of
                            select="element[@name = 'description']/element[@name = 'department']/element/field[@name = 'value']"/>
                        <xsl:text>.</xsl:text>
                    </subfield>
                </datafield>
            </xsl:if>
            ...
            <xsl:for-each
                select="element[@name = 'description']/element[@name = 'abstract']/element/field[@name = 'value'][1]">
            ...
            <datafield tag="504" ind1=" " ind2=" ">
                <subfield code="a">
                    <xsl:text xml:space="preserve">Includes bibliographical references.</xsl:text>
                </subfield>
            </datafield>
            ...
            <datafield tag="540" ind1=" " ind2=" ">
                ...
                <subfield code="u">
                    <xsl:value-of
                        select="element[@name = 'rights']/element[@name = 'uri']/element/field[@name = 'value']"
                    />
                </subfield>
            </datafield>
            ...
            <xsl:for-each
                select="element[@name = 'contributor']/element[@name = 'advisor']/element/field[@name = 'value']">
                <xsl:call-template name="persname_template">
                    <xsl:with-param name="string" select="."/>
                    <xsl:with-param name="field" select="'700'"/>
                    <xsl:with-param name="ind1" select="'1'"/>
            ...
            <xsl:for-each
                select="element[@name = 'contributor']/element[@name = 'committeemember']/element/field[@name = 'value']">
                <xsl:call-template name="persname_template">
                    <xsl:with-param name="string" select="."/>
                    <xsl:with-param name="field" select="'700'"/>
                    <xsl:with-param name="ind1" select="'1'"/>
            ...
            <xsl:variable name="bitstream"
                select="../element[@name = 'bundles']/element[@name = 'bundle']/element[@name = 'bitstreams']/element[@name = 'bitstream']/field[@name = 'description']"/>
            ...
            <xsl:choose>
                ...
                <xsl:when test="contains($string, '-')">
                    ...
                    <xsl:choose>
                        ...
                        <xsl:when
                            test="translate(substring($name_1, (string-length($name_1)), 1), '0123456789', '9999999999') = '9'">
                            <xsl:variable name="name"
                                select="substring($name_1, 1, (string-length($name_1) - 6))"/>
                            <subfield code="a">
                            ...
                            <xsl:choose>
                                ...
                                <xsl:when test="contains($remainder, '-')">
                                    ...
                                    <xsl:variable name="name_2"
                                        select="substring($tmp, 1, (string-length($tmp) - 6))"/>
                                    <subfield code="a">
                                        <xsl:value-of select="$name_1"/>-<xsl:value-of
                                            select="$name_2"/>
                                    </subfield>
                                </xsl:when>
            ...
            <xsl:if test="contains($string, '-')">
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="contains($remainder, '-')">
                        ...
                        <xsl:variable name="date_1"
                            select="substring($remainder, (string-length($tmp) - 3))"/>
                        <!-- CHECK WHETHER IT HAS A NUMBER BEFORE IT AND IF SO, OUTPUT IT AS DATE -->
                        <xsl:if
                            test="translate(substring($date_1, 1, 1), '0123456789', '9999999999') = '9'">
                            <subfield code="d">
                                <xsl:value-of select="$date_1"/>. </subfield>
                        </xsl:if>
                    </xsl:when>
                    <!-- OTHERWISE THIS IS THE ONLY DASH SO TAKE IT -->
                    <xsl:otherwise>
                        <xsl:variable name="date_2"
                            select="substring($string, (string-length($date_tmp) - 3))"/>
                        <!-- CHECK WHETHER IT HAS A NUMBER BEFORE IT AND IF SO, OUTPUT IT AS DATE -->
                        <xsl:if
                            test="translate(substring($date_2, 1, 1), '0123456789', '9999999999') = '9'">
                            <subfield code="d">
                                <xsl:value-of select="$date_2"/>. </subfield>
                        </xsl:if>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:if>
            <subfield code="e">
                <xsl:value-of select="$type"/>. </subfield>
            <xsl:if test="$orcid">
                <subfield code="0">
