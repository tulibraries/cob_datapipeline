...
def create_dag(env, version):
    ...
    CATALOG_SOLR_CONFIG = Variable.get("CATALOG_FULL_REINDEX_SOLR_CONFIG_" + env.upper(), deserialize_json=True)
    # {"configset": "tul_cob-catalog-0", "replication_factor": 2}
    CONFIGSET = CATALOG_SOLR_CONFIG.get("configset")
    ALIAS = CONFIGSET + f"-{env}"
    ...
    LATEST_RELEASE = bool(Variable.get(f"CATALOG_{env.upper()}_LATEST_RELEASE"))
    ...
    DAG = airflow.DAG(
        f"{env}_sc_catalog_full_reindex",
        catchup=False,
