...
cd ..
git clone --single-branch --branch master git@github.com:tulibraries/ansible-playbook-airflow.git # clone deployment playbook
...
pipenv run ansible-playbook -i inventory/stage/hosts playbook.yml --vault-password-file=~/.vault -e 'ansible_ssh_port=9229' --private-key=~/.ssh/.conan_the_deployer
EOF
