...
def alma_oai_harvest(**kwargs):
    try:
        ...
        data_dir = kwargs.get("AIRFLOW_DATA_DIR")
        endpoint_url = kwargs.get("ALMA_OAI_ENDPOINT") + "/request"
        # e.g. "https://temple.alma.exlibrisgroup.com/view/oai/01TULI_INST/request"
        oai_publish_interval = kwargs.get("ALMA_OAI_PUBLISH_INTERVAL")

        date_current_harvest = datetime.now()
        date_last_harvest = datetime.strptime(kwargs.get("ALMAOAI_LAST_HARVEST_DATE"), UTC_DATESTAMP_FSTR)
        ...
        logging.info("Harvesting {}".format(harvest_args))
        try:
            records = sickle.ListRecords(**harvest_args)
        except Exception as ex:
            logging.info(str(ex))
            logging.info("No records matched the date range given")
            records = []
        ...
        for r in records:
            ...
            if subrecord is not None:
                ...
                logging.info("subrecord issue?")
                logging.info(r.raw)
        outfile.write(newrootclosingtag)
        outfile.close()
        deletedfile.write(newrootclosingtag)
        deletedfile.close()
        logging.info("num_updated_recs {}".format(num_updated_recs))
        Variable.set("almaoai_last_num_oai_update_recs", num_updated_recs)
        logging.info("num_deleted_recs {}".format(num_deleted_recs))
