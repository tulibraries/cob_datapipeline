...
slackpostonsuccess = send_slack_notification(channel="blacklight_project", username="airflow", text=":partygritty: {{ execution_date }} DAG {{ dag.dag_id }} success: We started with {{ ti.xcom_pull(task_ids='get_num_solr_docs_pre')['response']['numFound'] }} and ended with {{ ti.xcom_pull(task_ids='get_num_solr_docs_post')['response']['numFound'] }} docs. {{ ti.log_url }}")
